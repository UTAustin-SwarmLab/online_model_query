...
class WaymoLatencyGymEnv(gym.Env):
    ...
    def step(
        self, action: int, _idx: int = None, reset: bool = False
        ...
        ### calculate reward
        current_idx = self.shuffle_idx[self.cnt % self.num_samples]
        ...
        if not reset:
            ...
            if self.nstep % 1000 == 0:
                ...
                print(
                    f"step: {self.nstep}/{self.num_samples}, {reset}, action: {action}, action list: {self.action_list}, reward: {reward}"
                )
            if self.nstep % save_freq == 0:
                self.mean_reward_dict[self.nstep] = self.cumulative_reward / self.nstep
        info["action_list"] = self.action_list
        info["cumulative_reward"] = self.cumulative_reward
        info["nstep"] = self.nstep
    ...
    def save_cum_reward(self):
        ...
        df.to_csv(
            f"synced_data/cumulative_reward/waymo_latency_step{save_freq}.csv",
            index=False,
        )
        return
