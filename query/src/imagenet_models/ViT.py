...
for id in range(1000):
    df1k.append([id, model.config.id2label[id], 0, 0, "ViT"])
df1k = pd.DataFrame(
...
df1k_pred = []
for id in range(1000):
    df1k_pred.append([id, model.config.id2label[id], 0, 0, "ViT"])
...
df_validation.to_csv("./synced_data/csv/imagenet/ViT_df_validation.csv", index=False)

df1k.to_csv("./synced_data/csv/imagenet/ViT_df1k.csv", index=False)
df1k_pred.to_csv("./synced_data/csv/imagenet/ViT_df1k_pred.csv", index=False)
