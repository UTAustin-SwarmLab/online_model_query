...
processor = AutoImageProcessor.from_pretrained(
    "google/mobilenet_v1_0.75_192", device=device
)
...
for id in range(1000):
    df1k.append([id, model.config.id2label[id], 0, 0, "mobilenet"])
df1k = pd.DataFrame(
...
df1k_pred = []
for id in range(1000):
    df1k_pred.append([id, model.config.id2label[id], 0, 0, "mobilenet"])
...
for i in range(inference_number):
    ...
    pred_label = logits.argmax(-1).item() - 1
...
df_validation.to_csv(
    "./synced_data/csv/imagenet/mobilenet_df_validation.csv", index=False
)

df1k.to_csv("./synced_data/csv/imagenet/mobilenet_df1k.csv", index=False)
df1k_pred.to_csv("./synced_data/csv/imagenet/mobilenet_df1k_pred.csv", index=False)
