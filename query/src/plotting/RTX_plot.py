...
def plot_rtx(save=False, ax_=None):
    ### load embeddings
    ...
    ### load PPO reward
    ppo = pd.read_csv(f"./synced_data/cumulative_reward/rtx_step{batch_size}.csv")
    ### pandas to numpy
    rewards_ppo = ppo["mean_reward"].to_numpy()[: rewards_opt.shape[0]]
    steps = ppo["Step"].to_numpy()[: rewards_opt.shape[0]]
    greedy = pd.read_csv(
        f"./synced_data/cumulative_reward/RTX_greedy_step{batch_size}.csv"
    )
    ...
    if save:
        ...
        ax.set_ylabel("Cumulative Negative Mean Action Error", size=25)
        ax.set_title("rtx", size=30)
        ax.set_ylim(-1.42, -1.13)
        ax.set_yticks([-1.4, -1.3, -1.2])
        plt.savefig(
